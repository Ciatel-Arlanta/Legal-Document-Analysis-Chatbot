...
def load_llm():
    try:
        return HuggingFacePipeline(pipeline=pipeline("text2text-generation", model="google/flan-t5-small", max_length=512, do_sample=True, temperature=0.3, device=0 if torch.cuda.is_available() else -1))
    except:
...
def load_question_generator():
    return pipeline("text2text-generation", model="google/flan-t5-small", max_length=200, temperature=0.7)
